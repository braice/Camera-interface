...
typedef enum {
  AT_SDKVersion          = 0x40000000, // Using large numbers to force size to an integer
...
unsigned int GetHardwareVersion(unsigned int* PCB, unsigned int* Decode, unsigned int* SerPar, unsigned int* Clocks, unsigned int* CameraFirmwareVersion, unsigned int* CameraFirmwareBuild);
...
unsigned int GetMCPGainRange(int* low, int* high);
...
unsigned int IsCoolerOn(int *_i_coolerStatus);
...
unsigned int SetNumberPrescans(int number);
